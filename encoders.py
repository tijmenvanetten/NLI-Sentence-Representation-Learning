...
class LSTMEncoder(nn.Module):
    def __init__(
        self, word_embed_dim, enc_n_layers, enc_h_dim, *args, **kwargs
    ) -> None:
    ...
    def forward(self, sentences_input):
        ...
        sent_packed = nn.utils.rnn.pack_padded_sequence(
            sent, sent_len, batch_first=True, enforce_sorted=False
        )
        _, (h_n, _) = self.lstm(sent_packed)
...
class BiLSTMMaxPoolEncoder(BiLSTMEncoder):
    ...
    def forward(self, sentences):
        ...
        sent_output = [
            embedding[:pad_idx] for embedding, pad_idx in zip(sent_output, sent_lens)
        ]
        sent_output_max_pool = torch.stack(
            [torch.max(x, 0)[0] for x in sent_output], axis=0
        )
